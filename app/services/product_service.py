...
def validate_product_payload(name, category_id, price, condition_level, description, trade_location, submit=False):
    ...
    if submit:
        ...
        if payload['price'] is None:
            return False, '请输入价格。', None
        if payload['price'] < Decimal('0.01'):
            return False, '价格必须大于或等于0.01。', None
        if payload['price'] > Decimal('99999.99'):
            return False, '价格必须小于或等于99999.99。', None
        ...
        # 保存草稿时宽松验证
        if payload['name'] and len(payload['name']) > 50:
            return False, '商品名称最多50个字符。', None
        ...
        if payload['description'] and len(payload['description']) > 1000:
            return False, '商品描述最多1000个字符。', None
...
def create_product(seller_id, name, category_id, price, condition_level,
    ...
    # 为草稿提供默认值
    product_name = payload['name'] or '未命名商品'
    product_price = payload['price'] or Decimal('0.01')
    product_condition = payload['condition_level'] or '全新'
    product_description = payload['description'] or '暂无描述'
    product_location = payload['trade_location'] or '待确定'
    product_category_id = payload['category_id']
    
    # 如果没有选择分类，使用第一个可用分类
    if not product_category_id:
        default_category = Category.enabled().order_by(Category.category_id).first()
        if default_category:
            product_category_id = default_category.category_id
        else:
            return False, '没有可用的商品分类，请联系管理员。', None
    ...
    product = Product(
        ...
        price=product_price,
        condition_level=product_condition, 
        description=product_description,
        trade_location=product_location, 
