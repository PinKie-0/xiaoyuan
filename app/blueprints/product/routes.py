...
def publish():
    # 检查是否有发布成功的标记
    publish_success = session.pop('publish_success', False)
    ...
    if form.validate_on_submit():
        ...
        if success:
            ...
            # 在 session 中设置标记，用户下次打开发布页面时会清空表单
            session['publish_success'] = True
            return redirect(url_for('product.my_products'))
        else:
            # 服务层验证失败，保留表单数据，显示错误
            flash(message, 'danger')
    
    # 只有当是第一次 GET 请求时才设置 token，POST 请求失败时保持原有 token
    if request.method == 'GET':
        form.submission_token.data = get_product_submission_token()
    
    return render_template('product/publish.html', 
                         form=form, 
                         categories=form.category_id.choices,
